...
class ThumbnailsView {
    ...
    private func layoutThumbnailViews(_ widthMax: CGFloat) -> (CGFloat, CGFloat, CGFloat)? {
        ...
        let startingX = isLeftToRight ? (DockAltTabMode ? 0 : Appearance.interCellPadding) : widthMax - (DockAltTabMode ? 0 : Appearance.interCellPadding)
        var currentX = startingX
        var currentY = (DockAltTabMode ? 0 : Appearance.interCellPadding)
        var maxX = CGFloat(0)
        var maxY = currentY + height + (DockAltTabMode ? 0 : Appearance.interCellPadding)
        ...
        while index < ThumbnailsView.recycledViews.count {
            ...
            if index < Windows.list.count {
                ...
                if needNewLine(projectedX, widthMax) {
                    currentX = startingX
                    currentY = (currentY + height + (DockAltTabMode ? 0 : Appearance.interCellPadding)).rounded(.down)
                    ...
                    maxY = max(currentY + height + (DockAltTabMode ? 0 : Appearance.interCellPadding), maxY)
                    rows.append([ThumbnailView]())
                } else {
                    view.frame.origin = CGPoint(x: localizedCurrentX(currentX, width), y: currentY)
                    currentX = projectedX
    ...
    private func projectedWidth(_ currentX: CGFloat, _ width: CGFloat) -> CGFloat {
        if App.shared.userInterfaceLayoutDirection == .leftToRight {
            return currentX + width + (DockAltTabMode ? 0 : Appearance.interCellPadding)
        }
        return currentX - width - (DockAltTabMode ? 0 : Appearance.interCellPadding)
    }
    ...
    private func layoutParentViews(_ maxX: CGFloat, _ widthMax: CGFloat, _ maxY: CGFloat, _ labelHeight: CGFloat) {
        ...
        // contentView.frame.size = NSSize(width: min(maxX, widthMax) + (DockAltTabMode ? 1 : frameWidth) * 2, height: min(maxY, heightMax) + (DockAltTabMode ? 1 : frameHeight))
        ...
        scrollView.frame.origin = CGPoint(x: (DockAltTabMode ? 1 : originX), y: (DockAltTabMode ? 1 : originY))
    ...
    func centerRows(_ maxX: CGFloat) {
        var rowStartIndex = 0
        var rowWidth = (DockAltTabMode ? 0 : Appearance.interCellPadding)
        var rowY = (DockAltTabMode ? 0 : Appearance.interCellPadding)
        for (index, window) in Windows.list.enumerated() {
            guard App.app.appIsBeingUsed else { return }
            guard window.shouldShowTheUser else { continue }
            let view = ThumbnailsView.recycledViews[index]
            if view.frame.origin.y == rowY {
                rowWidth += view.frame.size.width + (DockAltTabMode ? 0 : Appearance.interCellPadding)
            } else {
                shiftRow(maxX, rowWidth, rowStartIndex, index)
                rowStartIndex = index
                rowWidth = (DockAltTabMode ? 0 : Appearance.interCellPadding) + view.frame.size.width + (DockAltTabMode ? 0 : Appearance.interCellPadding)
